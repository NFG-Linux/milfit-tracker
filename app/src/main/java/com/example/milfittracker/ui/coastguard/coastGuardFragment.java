...
public class coastGuardFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,
        ...
        pushupPractice.setOnClickListener(v1 -> {
            ...
            args.putString("branch", "Coast Guard");
        ...
        startFullMock.setOnClickListener(v1 -> {
            ...
            NavController navController = NavHostFragment.findNavController(this);
            navController.navigate(R.id.coastguard_to_stopwatch, args);
        });
        ...
        new SetGoalRepo(requireContext()).getAllLive().observe(
                getViewLifecycleOwner(), goals -> {
                    for (SetGoal g : goals) {
                        if ("Coast Guard".equalsIgnoreCase(g.getBranch())) {
                            switch (g.getEvent()) {
                                case "Push-ups":
        ...
        vm.getAllLive().observe(getViewLifecycleOwner(), list -> {
            Scores latestPush = latestForBranch(list, "Push-ups", "Coast Guard");
            if (latestPush != null) pushupLast.setText("Last: " + latestPush.getEventValue());

            Scores latestPlank = latestForBranch(list, "Plank", "Coast Guard");
            if (latestPlank != null) plankLast.setText("Last: " + FormatTime.formatSeconds(latestPlank.getEventValue()));

            Scores latestRun = latestForBranch(list, "1.5-mile Run", "Coast Guard");
        ...
        btnStandards.setOnClickListener(vw -> {
            Bundle args = new Bundle();
            args.putString("branch", "CoastGuard");
            NavController navController = Navigation.findNavController(vw);
            navController.navigate(R.id.coastguard_to_standards, args);
        });
    ...
    private void showGoalDialog() {
        View dialogView = LayoutInflater.from(requireContext())
                .inflate(R.layout.set_goals_coast_guard, null);
        ...
        new AlertDialog.Builder(requireContext())
                .setTitle("Set Coast Guard Goals")
                .setView(dialogView)
                .setPositiveButton("Save", (d, w) -> {
                    ...
                    if (!pushStr.isEmpty()) {
                        ...
                        new SetGoalRepo(requireContext())
                                .save(new SetGoal("Coast Guard", "Push-ups", reps, "reps", null));
                    }
                    ...
                    if (!plankStr.isEmpty()) {
                        ...
                        if (secs > 0) {
                            new SetGoalRepo(requireContext())
                                    .save(new SetGoal("Coast Guard", "Plank", secs, "sec", null));
                        }
                    }
                    ...
                    if (!runStr.isEmpty()) {
                        ...
                        if (secs > 0) {
                            new SetGoalRepo(requireContext())
                                    .save(new SetGoal("Coast Guard", "1.5-mile Run", secs, "sec", null));
                        }
                    }
