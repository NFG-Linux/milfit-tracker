package com.example.milfittracker.ui.marines;
...
public class marineCorpsFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,
        ...
        View v = inflater.inflate(R.layout.fragment_marine_corps, container, false);
        ...
        pushupPractice.setOnClickListener(v1 -> {
            ...
            args.putString("branch", "Marine Corps");
        ...
        runPractice.setOnClickListener(v1 -> {
            Bundle args = new Bundle();
            args.putString("branch", "Marine Corps");
            args.putString("event", "3-mile Run");
            NavController navController = NavHostFragment.findNavController(this);
            navController.navigate(R.id.stopwatchFragment, args);
        });
        ...
        startFullMock.setOnClickListener(v1 -> {
            ...
            NavController navController = NavHostFragment.findNavController(this);
            navController.navigate(R.id.marines_to_stopwatch, args);
        });
        ...
        new SetGoalRepo(requireContext()).getAllLive().observe(
                getViewLifecycleOwner(), goals -> {
                    for (SetGoal g : goals) {
                        if ("Marine Corps".equalsIgnoreCase(g.getBranch())) {
                            switch (g.getEvent()) {
                                case "Push-ups":
        ...
        vm.getAllLive().observe(getViewLifecycleOwner(), list -> {
            Scores latestPush = latestForBranch(list, "Push-ups", "Marine Corps");
            if (latestPush != null) pushupLast.setText("Last: " + latestPush.getEventValue());

            Scores latestPlank = latestForBranch(list, "Plank", "Marine Corps");
            if (latestPlank != null) plankLast.setText("Last: " + FormatTime.formatSeconds(latestPlank.getEventValue()));

            Scores latestRun = latestForBranch(list, "3-mile Run", "Marine Corps");
        ...
        btnStandards.setOnClickListener(vw -> {
            Bundle args = new Bundle();
            args.putString("branch", "MarineCorps");
            NavController navController = Navigation.findNavController(vw);
            navController.navigate(R.id.marines_to_standards, args);
        });
    ...
    private void showGoalDialog() {
        View dialogView = LayoutInflater.from(requireContext())
                .inflate(R.layout.set_goals_marines, null);
        ...
        new AlertDialog.Builder(requireContext())
                .setTitle("Set Marine Corps Goals")
                .setView(dialogView)
                .setPositiveButton("Save", (d, w) -> {
                    ...
                    if (!pushStr.isEmpty()) {
                        ...
                        new SetGoalRepo(requireContext())
                                .save(new SetGoal("Marine Corps", "Push-ups", reps, "reps", null));
                    }
                    ...
                    if (!plankStr.isEmpty()) {
                        ...
                        if (secs > 0) {
                            new SetGoalRepo(requireContext())
                                    .save(new SetGoal("Marine Corps", "Plank", secs, "sec", null));
                        }
                    }
                    ...
                    if (!runStr.isEmpty()) {
                        ...
                        if (secs > 0) {
                            new SetGoalRepo(requireContext())
                                    .save(new SetGoal("Marine Corps", "3-mile Run", secs, "sec", null));
                        }
                    }
